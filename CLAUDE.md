...
- **Claude CodeがCodexのPRに関与するのは、①Codex側の環境要因で対応できない場合の検知、②レビューコメントを伴わない通常のCI失敗の検知とCodexへの再対応依頼（ChatGPT WorkはPR追跡を担わないため、CI失敗を拾う担当が抜け落ちないようにする）、③レビューが片付いた後の最終チェックの3点**。設計変更を伴う大きな指摘・アーキテクチャ判断はユーザーに戻す
