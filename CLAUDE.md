...
- **既存PRのレビュー再トリガー（`@codex review`）はPRコメントで直接行える**（新規タスクの割り当てとは異なりChatGPT Workの中継は不要）。**ただしコードの修正を伴う依頼（`@codex address that feedback`等）はPRコメント経由では行わない**：Codex側のサンドボックスにgit remote・GitHub認証が無くpushできないことが繰り返し確認されており、依頼のたびに失敗して手戻り・トークン消費が発生する。修正が必要な場合は、Claude Codeが新しい実装指示文を作りChatGPT Workの新規セッションで正式なCodexタスクとして依頼する。Claude CodeがCodexのPRに関与するのは、①軽微な指摘そのものへの一次対応（ユーザーの確認・適用）には関与しない、②レビューコメントを伴わない通常のCI失敗、③設計判断を要しない実装上の重大な不具合（correctnessバグ等）はClaude Codeが新しい実装指示文を作りChatGPT Work経由で再対応を依頼する、④Codex側の環境要因で対応できない場合の検知、⑤レビューが片付いた後の最終チェック。設計変更を伴う大きな指摘・アーキテクチャ判断はユーザーに戻す。**再対応依頼のpush後は、CIの再実行（本リポジトリはCIが無いため対象外）と最終HEADに対する再レビューの両方が完了するまで最終チェックへ進まない**
- **Codexの自動レビュー（Codex Review Bot）はGitHubのformal review／review threadではなく、PRトップレベルコメントとして投稿される場合がある**（2026-08-28、apps#390で確認）。マージゲート判定時は、直近のCodex Reviewコメントが対象とした「Reviewed commit」のSHAと、判定時点のPR最終HEADのSHAが一致していることを確認する。一致しない場合（HEAD更新後に古いSHAへのclean reviewしか無い場合）はMerge Readyとしない。**ただし、Codexは指摘が無い場合レビューコメントの代わりにPR本文へ👍リアクションのみを付ける方式のため、HEAD更新後に新しいCodex Reviewコメントが一切無い状態は「新HEADへのレビュー未実施」と「👍リアクションのみで完了」を区別できない。この場合は沈黙をclean判定の根拠にせず、現行HEADに対して`@codex review`を明示的に再依頼するか、ユーザーに画面で👍リアクションの対象コミットを確認してもらう、のいずれかで確定させてからMerge Ready判定を行う**（2026-08-28、[ai-workspace#153](https://github.com/kumacha666/ai-workspace/pull/153)のCodexレビュー指摘を受けて追記）
...
- **完了根拠はGitHub上の実体のみ**とする。対象PRブランチにコミットSHAと差分が実在することを確認してから完了扱いにする（Codexの自己申告・ローカル作業・未pushコミットは完了扱いにしない）。マージゲートは必ず最終HEADのSHA基準で判定し（最終HEADのレビュー結果〈Codexは指摘が無い場合レビューコメントの代わりにPR本文へ👍リアクションのみ付ける。Claude Codeの現行ツールではリアクションを直接取得できないため、コメント・スレッドが0件でもリアクション未確認ならユーザーに画面確認を依頼する〉／未解決のスレッド0件〈outdated化しても最終HEADでの解消根拠が無ければ未解決扱い〉／最終HEADに対応したテスト結果／ビルド成果物差分／上記Codex Review BotのReviewed commit SHA一致を確認）、古いコミットへのレビュー結果を流用しない（詳細な経緯・マージゲートの手順は`ai-workspace/CLAUDE.md`の「CodexのPR監視」参照）
