...
- **実装指示文の冒頭には「GitHub連携済みのkumacha666/appsを使い、ローカル作業フォルダへの事前接続を待たずGitHub上で直接確認する」旨を明記する**（非公開の`ai-workspace`はhandoffで既にサニタイズ済みの内容だけを含めるため、Codexへの指示文でこのリポジトリ名自体を挙げない。経緯は`ai-workspace/CLAUDE.md`の「振り分けと引き継ぎ」・`projects/honeypaw-lab/RETROSPECTIVES.md`参照）
