...
- **視覚的なUI崩れの検証**：CSS/DOM構造/アニメーションに関わる変更を含むPRで、対象アプリにPlaywright E2Eがある場合は、境界ケース（要素の重なりが起きやすい状態）を実際にレンダリングして検証する。E2Eが無いアプリで見た目に関わる変更をした場合は、その旨をPR本文に明記する。E2Eを持つアプリは`npm run test:e2e`をdeployパイプライン（`predeploy`等）に組み込み、実行を必須化する（実行を人任せにしない）
