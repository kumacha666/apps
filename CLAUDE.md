...
- **Codexの自動レビュー（Codex Review Bot）はGitHubのformal review／review threadではなく、PRトップレベルコメントとして投稿される場合がある。**トップレベルコメントには解決済み状態が無いため、直近1件だけでなく、それまでにCodexが投稿した全レビューコメントのうち未対応のcorrectness／regression／data loss・corruption／security／テストのfalse positive・false negativeに該当する指摘が最終HEADに対して残っていないかを確認する。SHA付きのレビューコメント（「Reviewed commit: `<SHA>`」を含むコメント）がある場合は、そのSHAと判定時点の最終HEADのSHAが一致することも確認する（一致しなければMerge Readyとしない）。CodexのP1/P2等のラベルは参考情報として扱い、それ自体を最終判断基準にしない。

  SHA付きのレビューコメントが確認できない場合（Codexが👍リアクションのみを付けた、または沈黙している場合）は、PRレベルの👍リアクション（`search_pull_requests`等の`reactions`で件数のみ取得可能）はコミット単位の証跡になり得ないため、件数・反応者のいずれで見てもclean判定の根拠に使わない。`@codex review`を明示的に再依頼した上で、対象HEADに対するSHA付きコメントが新たに投稿されるのを待つ。投稿されない場合はレビュー未確定として扱い、マージ判断が必要であればユーザーへ状況を報告し判断を仰ぐ。
