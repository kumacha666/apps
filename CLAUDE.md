...
- **視覚的なUI崩れの検証**：CSS/DOM構造/アニメーションに関わる変更を含むPRで、対象アプリにPlaywright E2Eがある場合は、境界ケース（要素の重なりが起きやすい状態）を実際にレンダリングして検証する。E2Eが無いアプリで見た目に関わる変更をした場合は、その旨をPR本文に明記する。**`enblo`/`enblo-classic`は`npm run build`（`scripts/build.mjs`）に`npm run test:e2e`が組み込み済み（2026-07-22対応、テスト失敗時はビルドが止まる）。E2Eがまだ無いアプリ（`combrawl`, `7metch`, `7metch2`, `mori-no-yakai`）は今後整備予定で、それまではPR本文に「E2E未整備」である旨を明記する**
