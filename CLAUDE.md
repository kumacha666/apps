...
- ランディングページ（`index.html`）に掲載されているのは現状 `7metch` と `emoji-dm` のみ。他のアプリは未掲載／開発中で、URLを直接踏んで動作確認する運用。新しいアプリを公開する際は `index.html` の `.apps` セクションにカードを追加する。`mori-no-yakai` はFirebaseプロジェクトのセットアップ（人間の手作業）が完了するまで未掲載（`mori-no-yakai/CLAUDE.md`参照）。
