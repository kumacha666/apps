...
- **`dusty-jukebox`は2026-08-31、実機で実際にGoogle Driveの音楽を再生できる状態に到達した**（PWA化済み、`manifest.json`/`sw.js`あり。CSSは`index.html`にインライン、`npm run deploy` は`scripts/copy-dist-app.mjs`経由での`dist/app.js`コピー＋SWバージョン自動更新（`scripts/update-sw-version.mjs`））。実クライアントID（`VITE_GOOGLE_CLIENT_ID`）を設定した環境で`npm run deploy`が実行され、OAuth認証・Drive走査・Sheets索引・`changes.list`差分同期・Service Workerストリーミング・単曲再生・絞り込み・連続再生キューまで一通り公開バンドル（`dusty-jukebox/app.js`、コミット`aaba428`）に反映済み。**公開後に2件の実機バグが見つかり修正済み**：①PR #392 — 再生のたびに強制的に再ログインを要求される致命的な認証ループ（`sw.js`が`URLSearchParams`由来の`playbackGeneration`を文字列のまま渡し`Number.isInteger()`検証に常に失敗していた型不一致が根本原因、加えて認証継続フロー自体の設計不備）。②PR #398 — 実際には再生できず「Failed to load because no supported source was found」で必ず失敗する不具合（`sw.js`が`fetch()`でGoogle Drive `alt=media`から206応答を中継する際、`Content-Range`/`Accept-Ranges`はブラウザのCORS safelist外でJavaScriptから読めず、不完全な206レスポンスをブラウザへ返していたことが原因。ユニットテストは`fetch`をモックするため検出できず、実機検証で初めて判明した）。E2Eモックでは検出できない類のクロスオリジンヘッダー可視性の問題があり得ることは他のアプリでも留意すること。
