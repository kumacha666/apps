...
- **Codexの自動レビュー（Codex Review Bot）はGitHubのformal review／review threadではなく、PRトップレベルコメントとして投稿される場合がある**（2026-08-28、apps#390で確認）。マージゲート判定時は、SHA付きのReviewコメント（「Reviewed commit: `<SHA>`」を含むコメント）が存在する場合に限り、そのSHAと判定時点のPR最終HEADのSHAが一致していることを確認する。一致しない場合（HEAD更新後に古いSHAへのコメントしか無い場合）はMerge Readyとしない。

  **SHA付きのReviewコメントが確認できない場合**（Codexが👍リアクションのみを付けた、または沈黙している場合）は、上記のSHA一致条件は適用せず、以下の手順に従う：PRレベルの👍集計（`search_pull_requests`等の`reactions`）はCodex Bot自身の反応か他の参加者の反応かを区別できず、同一Botアカウントは同一itemへ同じ絵文字を二重に付けられないため、件数の増減はclean判定の根拠にしない。
  1. `@codex review`を明示的に再依頼する（未依頼であれば）
  2. 対象HEADに対して「Reviewed commit: `<SHA>`」付きの新規コメント（Codexは指摘が無い「no major issues」の場合もこの形式で投稿する実績がある）が付くのを待つ
  3. 付いた場合はそのSHAとHEADの一致・内容で判定する（上記の基準に戻る）
  4. 沈黙が続く、またはPRレベルの👍集計しか確認できない場合は、clean判定の根拠にしない。マージ判断を急ぐ必要がある場合は、ユーザーに画面でCodex Bot自身の👍が現在のHEADに対して付いているかを確認してもらう（GitHub UIでは反応した個々のユーザー/Botを確認できるが、現行ツールでは個々の反応者までは取得できないため）

  （2026-08-28、apps#391のCodexレビュー指摘〈P1: 集計件数からclean判定を推測しない／P2: SHA一致要件はSHA付きコメントがある場合のみ適用する〉を受けて修正）
...
- **完了根拠はGitHub上の実体のみ**とする。対象PRブランチにコミットSHAと差分が実在することを確認してから完了扱いにする（Codexの自己申告・ローカル作業・未pushコミットは完了扱いにしない）。マージゲートは必ず最終HEADのSHA基準で判定し（最終HEADのレビュー結果〈SHA付きのReviewコメントがある場合はそのSHAとHEADの一致を、無い場合は上記フォールバック手順の結果を確認する〉／未解決のスレッド0件〈outdated化しても最終HEADでの解消根拠が無ければ未解決扱い〉／最終HEADに対応したテスト結果／ビルド成果物差分を確認）、古いコミットへのレビュー結果を流用しない（詳細な経緯・マージゲートの手順は`ai-workspace/CLAUDE.md`の「CodexのPR監視」参照）
