...
- **`dusty-jukebox`は2026-09-01、実機で実際にGoogle Driveの音楽を再生できる状態に到達した**（PWA化済み、`manifest.json`/`sw.js`あり。CSSは`index.html`にインライン、`npm run deploy` は`scripts/copy-dist-app.mjs`経由での`dist/app.js`コピー＋SWバージョン自動更新（`scripts/update-sw-version.mjs`））。2026-08-31、実クライアントID（`VITE_GOOGLE_CLIENT_ID`）を設定した環境で`npm run deploy`が実行され、OAuth認証・Drive走査・Sheets索引・`changes.list`差分同期・Service Workerストリーミング・単曲再生・絞り込み・連続再生キューまで一通り公開バンドル（`dusty-jukebox/app.js`、コミット`aaba428`）に反映されたが、**この時点ではまだ一度も再生できなかった**。**公開後に2件の実機バグが見つかり修正済み**：①PR #392（`aaba428`には反映済み）— 再生のたびに強制的に再ログインを要求される致命的な認証ループ（`sw.js`が`URLSearchParams`由来の`playbackGeneration`を文字列のまま渡し`Number.isInteger()`検証に常に失敗していた型不一致が根本原因、加えて認証継続フロー自体の設計不備）。②PR #398（2026-09-01マージ、`sw.js`は手書きソースのためマージと同時に公開反映） — 実際には再生できず「Failed to load because no supported source was found」で必ず失敗する不具合（`sw.js`が`fetch()`でGoogle Drive `alt=media`から206応答を中継する際、`Content-Range`/`Accept-Ranges`はブラウザのCORS safelist外でJavaScriptから読めず、不完全な206レスポンスをブラウザへ返していたことが原因）。**この種の不具合はfetchのモックで再現・検出可能だが、既存モックがこの欠落状態を再現していなかったため見逃されており**、実機検証で初めて判明した。外部APIをフェッチ・中継するコードでは、レスポンスヘッダーの一部がCORS safelist外で見えない可能性を意識し、その欠落状態を明示的にモックしたテストケースを用意すること（他のアプリでも留意）。
  - OAuth認証（トークンモデル、`drive.readonly`+`spreadsheets`スコープ）＋`drive.readonly`でのファイル一覧取得（`src/auth.ts`/`src/drive.ts`）、Sheets索引upsert（`src/sheets.ts`、`index`/`sync`タブ自動作成・重複行マージ）、実Drive `fetchRange`＋タグ抽出（`src/tagExtraction.ts`、`music-metadata`依存）、初回スキャンのバッチ処理・中断再開（`scanRunId`ウォーターマーク）、`changes.list`差分同期・リコンサイル（`src/differentialSync.ts`）、Service Workerストリーミングプロキシ・単曲再生（`sw.js`、PR #381）、絞り込み（artist/年/Genre）・除外・連続再生キューUI（次へ/前へ）まで実装済み。アルバム単位の通し再生・保存済みプレイリスト・検索窓・Album/Composer等の追加絞り込みは引き続き未着手（次セッションの候補、優先度は未検討）
  ...
  - `music-metadata`はフォーマットごとのパーサーを内部で`dynamic import()`するため、`vite.config.js`に`inlineDynamicImports: true`を設定し単一の`dist/app.js`に強制的にまとめている（このアプリの`npm run deploy`が`dist/app.js`1本のみをコピーする前提を壊さないため）。ログイン系機能を有効にするには`.env`に`VITE_GOOGLE_CLIENT_ID`（Web用OAuthクライアントID）の設定が必要（未設定時はビルドで自動的に除去され「未設定」表示になる）
