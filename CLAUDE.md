...
姉妹リポジトリ `kumacha666/ai-workspace`（private）でブレスト・設計を行い、本リポジトリで実装・公開する。設計・仕様検討の経緯は非公開の姉妹リポジトリ側にあり、本リポジトリからは参照できない。**本リポジトリのみにアクセスするAIエージェント（Codex等）向けの実行ルールは`AGENTS.md`（共通）＋各アプリの`AGENTS.md`（アプリ固有、存在する場合）に記載する**（2026-08-28導入。従来は本ファイルがCodexの実行契約も兼ねていたが、Codex向け実行ルールと本ファイル＝Claude Code向け設計・プロジェクト知識を分離した。現時点ではCodexに必要なルールの一部がまだ本ファイル側に残っているため、CodexはAGENTS.mdに加えて本ファイルおよび対象アプリのCLAUDE.mdも引き続き参照する移行期間中の暫定運用。詳細は`AGENTS.md`参照）。本CLAUDE.mdの「AI開発ルール」セクションの内容も、この移行が完了するまでは実質的にCodex向けルールを兼ねる。
...
**Codex向けの固定実行ルール（source of truth、リモート反映経路、完了条件、テスト方針、preflight証跡、デプロイ、PR運用等）は`AGENTS.md`（共通）＋各アプリの`AGENTS.md`（アプリ固有）に記載する（2026-08-28導入、[apps#390](https://github.com/kumacha666/apps/pull/390)）。以下の記述は主にClaude Code側の運用手順であり、Codexへの実装指示文はAGENTS.mdの内容を前提として目的・対象・受け入れ条件等のみを書く。**
...
- Codexへの実装指示文には、目的、対象/非対象、制約、受け入れ条件、実行すべきテスト、参照すべきアプリ固有CLAUDE.md（およびAGENTS.mdがあれば併記）を含める。非公開の背景は実装に必要な最小限だけ handoff または実装指示文に渡す
...
- **実装指示文の末尾には「PR作成後、PR番号またはURLを提示すること」を明記する**。Claude CodeはPRの新規作成を自動検知できないため、ユーザーがこの番号をClaude Codeに伝えた時点で監視（`subscribe_pr_activity`）を開始する
...
- **既存PRのレビュー再トリガー（`@codex review`）はPRコメントで直接行える**（新規タスクの割り当てとは異なりChatGPT Workの中継は不要）。**ただしコードの修正を伴う依頼（`@codex address that feedback`等）はPRコメント経由では行わない**：Codex側のサンドボックス内の書き込み経路（git remote・GitHub認証）が利用可能とは限らず、依頼のたびに失敗して手戻り・トークン消費が発生した実績がある（本ファイル`AGENTS.md`の「3. リポジトリへの反映経路」参照）。修正が必要な場合は、Claude Codeが新しい実装指示文を作りChatGPT Workの新規セッションで正式なCodexタスクとして依頼する。Claude CodeがCodexのPRに関与するのは、①軽微な指摘そのものへの一次対応（ユーザーの確認・適用）には関与しない、②レビューコメントを伴わない通常のCI失敗、③設計判断を要しない実装上の重大な不具合（correctnessバグ等）はClaude Codeが新しい実装指示文を作りChatGPT Work経由で再対応を依頼する、④Codex側の環境要因で対応できない場合の検知、⑤レビューが片付いた後の最終チェック。設計変更を伴う大きな指摘・アーキテクチャ判断はユーザーに戻す。**再対応依頼のpush後は、CIの再実行（本リポジトリはCIが無いため対象外）と最終HEADに対する再レビューの両方が完了するまで最終チェックへ進まない**
- **Codexの自動レビュー（Codex Review Bot）はGitHubのformal review／review threadではなく、PRトップレベルコメントとして投稿される場合がある**（2026-08-28、apps#390で確認）。マージゲート判定時は、直近のCodex Reviewコメントが対象とした「Reviewed commit」のSHAと、判定時点のPR最終HEADのSHAが一致していることを確認する。一致しない場合（HEAD更新後に古いSHAへのclean reviewしか無い場合）はMerge Readyとしない。

  **SHA付きのReviewコメントが無い状態（Codexが👍リアクションのみを付ける、または沈黙している状態）でのclean review確認は、以下の手順に統一する**（Codexの👍リアクションはPRレベル＝issueリアクションであり、どのコミットに対する反応かを特定できないため。件数自体は`search_pull_requests`等の`reactions`フィールドで取得できる）：
  1. `@codex review`を再依頼する前に、その時点の👍件数と最終HEAD SHAを記録する
  2. そのHEADに対して`@codex review`を明示的に再依頼する
  3. 再依頼後、👍件数が増加した、またはそのHEADを対象としたSHA付きレビューコメントが新たに付いたことを確認する
  4. ①の記録時点から③の確認時点までHEADが変わっていないことを確認する
  5. 👍件数が増えず新しいコメントも付かない（沈黙）場合、または再依頼前から存在する古い👍しか無い場合は、clean判定の根拠にしない
  6. ③でSHA付きのReviewコメントが返った場合は、従来どおりReviewed commit SHA == HEADで判定する

  （2026-08-28、apps#390のCodex自己申告不一致とai-workspace#153のCodexレビュー指摘を受けて、旧来の「ユーザーに画面で確認してもらう」手順から上記の機械的確認手順へ統一）
...
- **完了根拠はGitHub上の実体のみ**とする。対象PRブランチにコミットSHAと差分が実在することを確認してから完了扱いにする（Codexの自己申告・ローカル作業・未pushコミットは完了扱いにしない）。マージゲートは必ず最終HEADのSHA基準で判定し（最終HEADのレビュー結果〈Codexは指摘が無い場合レビューコメントの代わりにPR本文へ👍リアクションのみ付ける。👍の件数は`search_pull_requests`等の`reactions`フィールドで取得できるが、どのコミットに対する反応かは特定できないため、上記のclean review確認手順で判定する〉／未解決のスレッド0件〈outdated化しても最終HEADでの解消根拠が無ければ未解決扱い〉／最終HEADに対応したテスト結果／ビルド成果物差分／上記Codex Review BotのReviewed commit SHA一致を確認）、古いコミットへのレビュー結果を流用しない（詳細な経緯・マージゲートの手順は`ai-workspace/CLAUDE.md`の「CodexのPR監視」参照）
