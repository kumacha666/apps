...
- **PR作成後、ドラフト状態のままにせずready for review状態に変換する**（Claude Code側の仕様でPRは常にドラフトとして作成されるため、作成直後に変換操作を行う。2026-07-22追加）
...
- **コードレビュー（Codexの自動レビューに代わる社内チェック、2026-07-22追加）**：PR作成後、`/code-review` でセルフレビューを実行し、PR本文のTest planに実行有無をチェック項目として明記する（例：`- [x] /code-review 実施済み`）。実行を強制する仕組みは無いため、マージ判断をする人間がPR本文でチェック有無を確認する運用に乗せる（本リポジトリは元々マージを人間が判断するため、この可視化は判断を素早くするための補助）
  - **`/code-review`はAIが自分自身で起動できない**（`disable-model-invocation`設定のため、`Skill`ツール経由の呼び出しは常に拒否される）。PR作成後、AIはユーザーに実行を依頼する運用とする（2026-07-24確認）。依頼する際は「プロンプト欄に直接入力してください」と伝えること。チャット上に貼り付けられた`/code-review`という文字列をAIが受け取って実行しようとしても同じ理由で失敗する（**コピペでは起動しない**、ユーザーの入力欄からの直接入力のみ有効。2026-07-24、Claude Code on the webのセッションで実際に確認済み）
  - **`/code-review`の出力は英語になる**（プロジェクトの「日本語でやりとり」方針に関わらず、スキル自体の既定言語のため）。ユーザーがコマンドを実行した後、AIは結果を日本語で要約してから対応方針を提示すること（2026-07-24追加）
  - **`--comment`オプションは付けなくてよい（2026-08-03確認）**：以前は`/code-review --comment`を推奨していたが、`--comment`はGitHub関連ツールにアクセスできず機能しない（スキル自身の出力で「インライン投稿ができないためテキストで出力する」というフォールバックが明記される）。実際にはPRへの反映は、出力を見たAIが`mcp__github__`ツールで手動投稿している。オプションの有無で結果は変わらないため、以後は単に`/code-review`でよい
