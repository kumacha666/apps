...
- **既知の問題（2026-09-16発見、未修正）**：`src/audio.ts`に`npm run build`（`tsc`）を失敗させる型エラーがある。`enblo`から凍結時点でそのままコピーされた行のため、詳細・原因は`enblo/CLAUDE.md`の同項目を参照。凍結アプリのため優先度は低いが、`enblo`側を先に修正した場合はこちらへの反映（バグ修正のみの最小対応）も検討すること
