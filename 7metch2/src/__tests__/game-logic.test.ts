...
import { getStageTarget, getStageMoves, getStageBoardSize, getStageNumColors } from "../game";
...
describe("Stage target and moves", () => {
  ...
  it("board size grows with stage (every 3 stages)", () => {
    expect(getStageBoardSize(0)).toEqual({ cols: 7, rows: 8 });
    expect(getStageBoardSize(3)).toEqual({ cols: 8, rows: 9 });
    expect(getStageBoardSize(10)).toEqual({ cols: 10, rows: 11 });
    expect(getStageBoardSize(30)).toEqual({ cols: 17, rows: 18 });
  });

  it("num colors increases with stage", () => {
    expect(getStageNumColors(0)).toBe(5);
    expect(getStageNumColors(7)).toBe(5);
    expect(getStageNumColors(8)).toBe(6);
    expect(getStageNumColors(14)).toBe(6);
    expect(getStageNumColors(15)).toBe(7);
  });
});
