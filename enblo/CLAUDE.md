...
- **既知の問題（2026-09-16発見、未修正）**：`src/audio.ts`に`npm run build`（`tsc`）を失敗させる型エラーがある。`let channelStates: ChannelState[][] = [];`と宣言されているが、実際に代入・参照されているのは`ChannelState[]`（`channelStates[ci]`を単一の`ChannelState`として`.nextTime`/`.noteIdx`等で直接使っている）で、型注釈の`[][]`（二重配列）が実際の使い方と食い違っている。`npm test`（Vitest）は通るため今まで気づかれていなかった（`tsc`はビルド時にしか走らず、直近まで`npm install`自体がリポジトリ全体の依存関係バグでブロックされていたため誰も`npm run build`を実行できていなかった可能性が高い。詳細は`apps/CLAUDE.md`の「`vitest`は全B系アプリで`^3.2.7`を使う」参照）。`enblo-classic`（`enblo`の凍結時点のコピー）にも同じ行が存在する。修正自体は`ChannelState[][]`→`ChannelState[]`という1行の型注釈修正で足りると見られるが、実際に音声再生の動作確認をしていないため、次にこのアプリへ着手するセッションで意図を確認してから直すこと
