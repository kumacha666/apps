...
`ai-workspace/projects/enblo/` 配下のブレストドキュメントで、戦闘システムの再設計が検討済み・実装待ちの状態。詳細は `ai-workspace/projects/enblo/REMAINING_TASKS.md` を参照。要点:
...
- **`enblo-v1-prototype-snapshot` ブランチ**（コミット`e307028`）に、大改修着手前の完成形（試作品v1）を固定保存済み。旧6ステータス・旧ダメージ式・血統システム（共通血統+血統の極意）・タイムアウト=引き分け仕様を含む、いつでも遊べる状態のスナップショット。このブランチは削除・上書きしないこと
