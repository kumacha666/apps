...
- [x] **オービットPhase 2**: パターン消し進捗トラッキング（`src/patternClear.ts`, `src/patternClear.test.ts`）。パターン座標定義（外周/中央十字/対角線/四隅3x3）、進捗の累積判定、リカバリー処理・手動シャッフルアイテム由来の除去の除外、特殊ピース生成マスの計上（呼び出し側がclearedCellsに含めれば計上される設計）。既存のゲームループには未配線
