...
- **オービットPhase 4**: `GameState`への統合・実配線。ここで初めてStage 501以降の実プレイに反映される。規模が大きいため複数PRに分割する（各ステップでStage 1〜500の既存138件超のテストが全件パスすることを確認しながら進める）。
  ...
  - [ ] **4b-2b（新設、2026-08-12）. シミュレーターへの配線**: `scripts/simulate.mjs`の`findValidMoves()`・詰み判定を、共有の「有効な1手」列挙（`hasAnyLegalMove()`・オービット対応の通常スワップ・タップ起動・スワップ起動系特殊ピース）に揃える。**現状の`findValidMoves()`は通常マッチ成立スワップしか見ておらず、オービット制約も考慮していない**ため、オービットステージでは詰み率を過大評価しうる。加えて、合法手0件を検出した時点でシミュレーションを終了させるのではなく、本番の`finishTurn()`と同じ回復フロー（`shuffleWithQualityGate()`→ダメなら`regenerateBoardForDeadlock()`）を実行し、それでも解消しない場合のみ「詰み」として終了する必要がある（GIMMICK_REDESIGN.md参照）。タップ起動・スワップ起動系の「手」をシミュレーター側で実際に実行するには、`doMove()`のコンボ判定・起動処理に相当するロジックをシミュレーター側にも用意する必要があり、4b-2本体（ゲーム本編への配線）とは切り離して個別に検証する
