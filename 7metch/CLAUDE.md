...
- [ ] **オービットPhase 4**: `GameState`への統合・実配線。ここで初めてStage 501以降の実プレイに反映される。
  ...
  - **`boardSizeForStage()`（stages.ts）をStage 501以降向けに分岐させる**（`/code-review`指摘、2026-08-11）: 既存の`boardSizeForStage()`はステージindex 250以上で9x10を返す成長カーブロジックだが、`orbitStageGen.ts`のフォールバックデータ・テストはパイロットの固定7x8盤面を前提にしている（設計ドキュメントで「新ギミックの検証に集中するため盤面成長は入れない」と明記済み）。Stage 501以降（index>=500）は既存の成長ロジックを流用せず、固定で7x8を返すよう分岐を追加すること。これを怠ると、素朴に既存関数を流用した場合に9x10盤面に対して`"1:8x7"`等のフォールバックキーがヒットせず、`getFallbackOrbitLayout()`が例外を投げてステージ生成が失敗する
