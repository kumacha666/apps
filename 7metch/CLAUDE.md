...
- [ ] **オービットPhase 4**: `GameState`への統合・実配線。ここで初めてStage 501以降の実プレイに反映される。
  - `isSwapLegal`をヒント/手動シャッフル/自動デッドロック回復/シミュレーターに配線（すべて同じ「有効な1手」の定義を共有）
  - `patternProgress`を`GameState`に追加してステージ開始時に初期化し、`recordClear`を共通の消去処理経路（通常マッチ・特殊ピース起動・アイテム使用）に配線する
  - `createBoard()`/`countAvailableMoves()`にオービット制約を適用し、「オービット制約適用後の合法手数が2〜10の範囲」の盤面品質チェックを実装する
  - `generateOrbitLayout()`をステージ生成フローに配線し、Stage 501以降で実際にオービットが配置されるようにする
  - **`boardSizeForStage()`（stages.ts）をパイロット範囲向けに分岐させる**（`/code-review`指摘、2026-08-11、Codexレビューで追加指摘・2026-08-11）: 既存の`boardSizeForStage()`はステージindex 250以上で9x10を返す成長カーブロジックだが、`orbitStageGen.ts`のフォールバックデータ・テストはパイロットの固定7x8盤面を前提にしている（設計ドキュメントで「新ギミックの検証に集中するため盤面成長は入れない」と明記済み）。**この固定7x8はパイロット規模（Stage 501〜524、`500 <= index < 524`）に限定した決定であり、Stage 525以降（パイロット後の本採用分。分量・盤面サイズは未検討・未決定）にまで恒久的に適用してはならない**。分岐条件は`index >= 500`ではなく`500 <= index && index < 524`とし、Stage 525以降の盤面サイズ方針は別途人間が判断する。これを怠ると、素朴に既存関数を流用した場合に9x10盤面に対して`"1:8x7"`等のフォールバックキーがヒットせず、`getFallbackOrbitLayout()`が例外を投げてステージ生成が失敗する（パイロット範囲内の場合）か、あるいはパイロット後の本採用分にまで固定7x8が意図せず波及する（範囲を`index>=500`のまま無条件にした場合）
- [ ] **オービットPhase 5**: チュートリアル（Stage 501初出時、既存の`tutorialDone`マップにキー500を追加するだけでよい。独立フラグは不要）・あそびかた画面への追加（既存の旧ギミック説明は残したまま追加）
- [ ] **オービットPhase 6（優先度: 低・任意）**: 分析基盤対応。`projects/7metch-analytics/`（非公開の姉妹リポジトリ）はStage 1〜500分のペイロード・レポートは変更せず、Stage 501以降専用のフィールド・分岐を追加する。**`stage-analyzer.js`（同じく非公開の姉妹リポジトリ内、`TOTAL_STAGES=350`のハードコードによりStage 351〜500は元々分析できない状態）は対応不要**（2026-08-11決定。Stage 1〜500は現状維持でよく、このツールでStage 351〜500を見られるようにする必要は無い。Stage 501以降の分析が必要になったタイミングでそのときだけ新体系用の分岐を追加すればよい）。新ステージ（Stage 501以降）のバランス検証は本来`npm run sim`を使う想定で、この分析ツールに実装（Phase 1〜5）が依存することはない
