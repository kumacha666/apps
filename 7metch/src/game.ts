...
async function handleCountdownExplosions(exploded: [number, number][]): Promise<void> {
  if (exploded.length === 0) return;
  SFX.bomb();
  const cleared = new Set<number>();
  for (const [r, c] of exploded) {
    cleared.add(r * G.cols + c);
    const queue: [number, number][] = activateSpecial(r, c, cleared, "countdown");
...
export async function activateByTap(r: number, c: number): Promise<void> {
...
export function activateCombo(comboType: ComboType, r: number, c: number, p1: Piece, p2: Piece): [number, number][] {
  ...
  switch (comboType) {
    ...
    case "rainbow_bomb": {
      ...
      const spType: SpecialType = comboType === "rainbow_line" ? "line_h" : "bomb";
      for (let rr = 0; rr < G.rows; rr++) {
        for (let cc = 0; cc < G.cols; cc++) {
...
export async function resolveMatches(): Promise<void> {
  ...
  while (matches.length > 0) {
    ...
    const specialInfos: SpecialInfo[] = [];
    matches.forEach(([r, c]) => {
...
export async function resolveBoard(): Promise<void> {
  await resolveMatches();
  const exploded: [number, number][] = tickCountdowns();
...
export function updateItemBar(): void {
  const coins = G.saveData.coins || 0;
  const el = G.dom!.itemCoinCount;
  if (G.debugMode) {
    if (el) el.textContent = "∞";
    document.querySelectorAll<HTMLButtonElement>(".item-btn").forEach(btn => { btn.disabled = false; });
    return;
  }
...
export function cancelItemMode(): void {
  G.itemMode = null;
  G.canvas!.classList.remove("item-targeting");
}

export async function usePinpoint(r: number, c: number): Promise<void> {
...
export function spawnSpecialAt(r: number, c: number, type: string): void {
  ...
  if (type === "countdown") {
    ...
    G.board[r][c]!.special = type as SpecialType;
  }
  drawBoard();
}
...
export function checkWinLose(): void {
  ...
  if (cleared) {
    ...
    if (isFirstClear && STAR_GATES.some((g: StarGate) => g.stage === G.currentStage)) {
      G.coinsEarned += 5;
    }
