...
// 依存しない）
export class StageStartQueue {
  private running = false;
  private pending: number | null = null;

  // 実行中でなければ呼び出し元に実行許可を与えてtrueを返す（内部状態をrunning=trueに
  // する）。既に実行中ならindexを「最新の保留要求」として記録するだけでfalseを返す
  // （複数回連続で呼ばれても、常に最後に渡されたindexだけが残る＝上書き）
  requestStart(index: number): boolean {
    if (this.running) {
      this.pending = index;
      return false;
    }
  ...
  // 続けて次のindexを処理する）。保留要求が無ければrunning状態を解放しnullを返す
  takeNextOrFinish(): number | null {
