...
// (dusty-jukebox本体と同じ方針。詳細はそちらのコメント参照)。Do not infer
// Playwright's cache layout here: chromium.executablePath() only identifies the
// full Chromium binary, while Playwright can run with only its headless shell
// installed. Outside this sandbox's stable shortcuts, attempt Playwright's own
// installation command so an unavailable browser fails the build instead of
// silently skipping the E2E gate.
