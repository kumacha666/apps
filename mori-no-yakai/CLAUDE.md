...
- **`myRoleBanner()`（自分の役職の固定表示）はnight画面では使わず、`myNightRoleBanner()`を使うこと**（`src/ui/context.ts`、2026-07-11、Codexレビュー指摘）。`myRoleBanner()`はcurrentRole基準のため、きつねの交換が完了した瞬間に交換対象になったプレイヤーのバナーがまだ夜が明けていないのに書き換わり、「今きつねに交換された」と伝わってしまう。`myNightRoleBanner()`はoriginalRole基準で夜フェーズ中は不変にしている（交換した本人には、きつねの行動画面自体で新しい役職が明示されるので支障はない）。discuss/vote/result画面は夜が明けた後の「最終的な役職」を見せる場面なので、引き続き`myRoleBanner()`（currentRole基準）を使う
