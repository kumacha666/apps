 ...
 *
 * かつては`markOnline()`の書き込み完了を待たずに早期進行チェックを呼ぶと、復帰した
 * 本人がまだ`online:false`のまま早期進行チェックが部屋を読みに行ってしまい、他の全員が
 * 既にタップ/投票済みだった場合、オンライン参加者の集計から復帰した本人が除外されたまま
 * 「全員完了」と判定されてフェーズが進んでしまう不具合があった（2026-07-11、
 * Codexレビュー指摘）。2026-08-16、早期進行の完了判定（`isNightStepComplete`/
 * `isDiscussComplete`/`maybeCloseVoteEarly`）を「配札済み全員」基準に変更し
 * オンライン状態を問わなくなったため、この特定の競合自体はもう起こらない。
 * ただし`markOnline()`の完了を待ってから他の処理を行う順序自体は、onDisconnect
 * ハンドラの再登録を確実に済ませてから進めるという意味で引き続き保つ。
 */
