...
- **全体攻撃化のヒット中は「全体化◯%」の常駐バッジを攻撃者の頭上に表示する（2026-07-17実装、GAME_DESIGN.md §2.3）**：`animateHits`の各グループ再生時に、そのヒットで実際に効いているトレイト倍率を`combat.ts`の`attackBadgeInfo()`から取得し、`main.ts`の`popAttackBadge(attackerEl, info, attacker)`が`.aoe-hit-badge`として攻撃者の上に表示する。ダメージポップと同じ720msだけ表示して消える（常駐と言っても「そのヒットが出ている間だけ」の意味で、ずっと表示され続けるわけではない）。フォントサイズは連撃のNヒット表示と同じ`fontSizeForHitIndex(hitIndex)`（`combat.ts`に切り出し済み、`hitDampen`等と同じくDOM非依存の純粋関数でテスト済み）を`fontSizeForHitIndex(level - 1)`として流用し、Lvが高いほど大きく見せる。以前は常駐バッジ`🌀N`（Lv数字のみ）とカード取得時のトーストでしか%を確認できず、実戦闘中に威力を実感できなかった
- **反撃のヒット中も同様に「反撃◯%」バッジを表示する（2026-07-17実装）**：`attackBadgeInfo()`が`retaliateMultFor(level)`（`combat.ts`）から算出した%を返し、`popAttackBadge`が表示する。**反撃の威力%計算式は`combat.ts`の`retaliateMultFor`に一本化済み**：以前は`data/cards.ts`にカード説明文表示専用の同一ロジック`retaliateMultForDisplay`が重複定義されていたが、全体化の`aoePercentForLevel`統一と同じ理由（片方だけ直すと表示と実ダメージがズレる事故を防ぐため）で削除し、`data/cards.ts`・`main.ts`・`battle.ts`の全てが`combat.ts`の`retaliateMultFor`を参照するようにした
- **全体化%と反撃%が同時に有効な場合（反撃ターンに全体化持ちが反撃するケース）は、内訳＋実際の合計を2段で正しく表示する（2026-07-20実装）**：以前は`animateHits`の呼び出し側で`kind === "retaliate"`のときは反撃%バッジのみ、それ以外（通常の攻撃ターン）は全体化%バッジのみを出す分岐になっており、両方が同時に有効な振り（反撃ターン中に全体化も乗っているケース）でも常にどちらか一方しか表示されず、**実際のダメージ倍率（両者の積）とバッジの数字が一致しない**不具合だった（ユーザー報告）。修正後は`combat.ts`の純粋関数`attackBadgeInfo({ aoeLevel, retaliateLevel, isRetaliateSwing })`が実ダメージ計算と同じ`aoePercentForLevel`/`retaliateMultFor`を使って「そのヒットで実際に効いている倍率」を判定し、両方有効なら`{ aoePercent, retaliatePercent, comboDeltaPercent }`（`comboDeltaPercent`は両者の積を基準100%からの増分で表した値、例: 170%×140%=238%→138）を返す。`popAttackBadge`はこれを受けて、内訳（`.attack-badge-breakdown`、小さく薄い色）を上段に、合計（`攻撃力+138%`のように通常サイズ）を下段に2枚同時表示する。**連撃の発ごとの減衰（`hitDampen`）はこのバッジには含めない**：全体化%・反撃%は1回の振りの中で一定の「build由来の倍率」だが、連撃の減衰は発ごとに変わる値で、既にその発のダメージポップ数値自体に反映されているため、性質の異なる情報を混ぜて逆にわかりにくくしないための意図的な設計（ユーザーとの相談で確認済み）
- **全体化%/反撃%/GUARD（挑発ブロック時表示）バッジの位置は`main.ts`の`badgeAnchorRect(unitEl)`で統一している（2026-07-17実装）**：常駐の特性バッジ`.unit-badges`（連撃⚡/全体化🌀/反撃↩/挑発🛡のいずれかを持つユニットなら必ずユニット直上に存在する）がユニットの真上を専有しているため、これらのヒット中バッジを素朴にユニット自身の真上に出すと常駐バッジと重なって読めなくなる（2026-07-17、ユーザー報告）。`badgeAnchorRect()`は`.unit-slot`内に`.unit-badges`があればその上端、無ければユニット自身の上端を返し、`popAttackBadge`/`popGuardBadge`/`popShieldShatter`は全てこの座標を基準にする。**新しくユニット頭上に浮かせる演出を追加する場合、`getBoundingClientRect()`をユニット要素へ直接呼ばずに、必ず`badgeAnchorRect()`を経由すること**（同じ罠を再発させないため）
- **GUARDバッジ（挑発ブロック時）は`popGuardBadge(targetEl)`で表示する**：以前は`popDamage(tEl, "GUARD", "#5ec8ff", null)`（通常のダメージポップと同じ、上に流れて消えるアニメーション）だったが、全体化%/反撃%バッジと表現が異なり一貫性が無く、かつ常駐バッジとも重なっていた（2026-07-17、ユーザー報告）。`popAttackBadge`と同じ`.aoe-hit-badge`クラス＋`badgeAnchorRect()`の位置・アニメーションに統一し、色だけ`.guard-hit-badge`（青白、`.guard-flash`と同系色）で差し替えている
...
- **「先鋭化」「硬質化」を実装済み**：単体選択、それぞれ`u.atk *= 4`／`u.def *= 3`のみ。硬質化は`u.dmgTakenMult`も同時に再計算する（先鋭化の倍率は後述「攻撃系3カードの強さ揃え」参照、当初は×3だったが2026-07-19に×4へ変更）
...
- **「挑発」を全面再設計（実装済み）**：被ダメ軽減%は廃止し、「1ラウンドにつきtauntLevel回ぶんダメージを完全無効化（0ダメージ）、ラウンドごとにリフィル」という新メカニクスに変更した。実装のポイント：
  ...
  - **アグロ（引きつけ）はブロック予算の有無に関係なく常に機能する仕様（GAME_DESIGN.md §2.5：「敵の攻撃（通常・連撃問わず）を優先的に受ける」）だが、実装当初は予算切れの瞬間にフォールバック先が「生存者全体」になっており、taunterが他ユニットと同列にランダム化されてしまう不具合があった（2026-07-20、ユーザー報告：「挑発が連撃に機能していない」）。連撃のような多段攻撃で1スイングの途中で予算切れになると、残りの発が他ユニットに逸れる形で顕在化していた。`enemyAttackTurn()`の`selectTargets`を「①予算が残っているtaunter→②（いなければ）taunter全体→③（taunterが1体もいなければ）生存者全体」の3段階フォールバックに修正した。**予算切れ後のフォールバックは必ず「taunter全体」に留め、「生存者全体」まで広げないこと**（同じバグの再発防止）
...
- **見た目（HP=サイズ/ATK=形/DEF=材質、12段階）を実装済み（2026-07-17）**：
  ...
  - **1段階に必要な伸び幅（`STEP_LOG2`）を3に変更し、ギャラリー全解放をレア化した（2026-07-18、ユーザー要望）**：`tierForValue(value, base, stepLog2)`の`stepLog2`引数（既定1＝約2倍ごとに1段階）を3にすると「約8倍ごとに1段階」になる。単体強化カード（特大化/硬質化は対象ステータスを×3、先鋭化は×4。後述「攻撃系3カードの強さ揃え」参照）を1つの軸に集中して積み続けた場合、以前（`STEP_LOG2=1`）は約7回で最大段階(tier12)に到達していたが、変更後は約20〜21回必要になる計算（特大化/硬質化は`3^N倍 >= 8^11倍`、先鋭化は`4^N倍 >= 8^11倍`を解くと算出できる。先鋭化はやや少ないピック数で到達する）。通常クリア（10〜20層）の範囲では埋まりきらず、エンドレスモードでの長期周回が前提の「エンドゲームでじっくり集める」難易度を意図している。全12段階のギャラリー解放しきい値を個別に調整したい場合（HP/ATK/DEFで別の重さにする等）は、`hpTier`/`atkTier`/`defTier`にそれぞれ別の`stepLog2`を渡すよう変更すればよい（現状は3チャンネルとも共通の`STEP_LOG2`定数）

- **攻撃系3カード（連撃の型・全体攻撃化・先鋭化）の強さを揃えた（2026-07-19実装）**：3枚提示（`showCardChoices()`が10種から3枚ランダム提示する仕様、`main.ts`参照）を模したシミュレーションで、「◯◯のみ優先ピック」時のクリア率を比較したところ、当初は連撃が頭一つ抜けて強く（合体を早めに使う前提で約27〜33%）、先鋭化はほぼ同水準（約24〜32%）だったが、全体化だけ明確に見劣りしていた（約16〜20%）。連撃の仕様（`combat.ts`の`hitDampen`）はそのままに、以下2枚を強化して3カードの単体クリア率・2枚組み合わせのクリア率をおおむね横並びにした：
  - **先鋭化**：ATK倍率 ×3 → **×4**
  - **全体攻撃化**：1ピックあたりの`aoeLevel`増分 1 → **5**（カード説明文「Lv1:80%→Lv2:95%→Lv3:110%…」も「1回でLvが5上がり威力上昇」に変更。1ピックでLvが5つ飛ぶため、旧来の「Lv1ずつ上がる」文言のままだと実態と合わなくなるため修正した）
  - シミュレーション結果（600ラン、10/20/40層目標）：単体優先ピックで連撃30.8/33.5/31.2%・先鋭化30.3/31.5/31.8%・全体化31.7/29.5/28.0%とほぼ横並び。2枚組み合わせ（連撃+全体化・連撃+先鋭化・全体化+先鋭化）も42〜49%の範囲に収まり、特定の組み合わせだけが突出することもないことを確認済み
  - この強化に伴い、`aoeLevel`は1ピックで大きく飛ぶ値になった。今後`aoeLevel`を参照する新しい表示・演出を追加する場合、「Lvは1ずつ増える」という前提を置かないこと
...
- **ギャラリー（メタ進行）を実装済み（2026-07-17）**：タイトル画面の「🖼 ギャラリー」ボタンから開ける、HP/ATK/DEFそれぞれ独立した12マスの到達record画面
  ...
  - **未到達セルにも到達に必要な数値の目安を表示する（2026-07-18実装）**：`visuals.ts`に`tierForValue`の逆関数`thresholdForValue(tier, base, stepLog2)`（往復検証込みでテスト済み）と、`hpThresholdForTier`/`atkThresholdForTier`/`defThresholdForTier`を追加した。`main.ts`の`buildGalleryTrack()`が未到達セルに「？」と一緒に`formatCompactNumber()`（K/M/B/T単位で短縮表示、`toLocaleString()`の桁区切りだと約63pxのセル幅に収まらないため専用実装）でしきい値を表示する（例: 「68.7B〜」）。ユーザー要望：「それぞれHP、ATK、DEFがいくつ以上でその見た目になるのか、？状態でも出してほしい。目指すべき指標になる」
...
- コミット前に`npm run deploy`を実行し、ビルド成果物（`game.js`/`style.css`/`manifest.json`/`index.html`）をルート直下に反映させる（`npm run build`は`scripts/build.mjs`が entry書き換え→処理→復元を「unit test」「E2E」「vite build」の3つの独立した窓に分けて順に実行し、失敗時も必ずindex.htmlを復元する。`npx vite build`を直接叩かないこと。2026-07-15、apps#298。2026-07-23、Codexレビュー指摘: 1つの大きな窓でまとめて囲むと`test:e2e`内部（`test-e2e.mjs`が単独実行時にも安全なよう自前でrestore/resetする）のreset-entryが早期発火し、直後のvite buildが未復元のindex.htmlを見てしまうバグがあったため3窓構成に分割）
