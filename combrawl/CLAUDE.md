...
- **「挑発」を全面再設計**：被ダメ軽減%は廃止し、「1ラウンドにつきtauntLevel回ぶんダメージを完全無効化（0ダメージ）、ラウンドごとにリフィル」という新メカニクスに変更。実装上の重要な注意点（掘り下げ済み、必ず`GAME_DESIGN.md`§7・§2.5を先に読むこと）：
...
- **「全体攻撃化」の威力%上限を撤廃**：`combat.ts`の`aoeMultFor`を次の式に置き換える（`aoePercentForLevel(level)`という名前を想定）：
  - `base = 0.65 + 0.15 * min(level, 5)`（Lv1:80%→Lv2:95%→Lv3:110%→Lv4:125%→Lv5:140%、旧`aoeMultFor`と同じ伸び）
...
- **「合体」「分裂」にDEFの継承ルールを追加**：DEFはHP/ATKと同じ「肉体枠」として扱う（合体は合算×1.2、分裂は0.6倍・下限1）。`dmgTakenMult`は個別に合算/コピーせず、継承後の`def`から導出する
