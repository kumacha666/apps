...
- **全体攻撃化（AoE）は同一hitIndexの複数ターゲットを同時ヒットとして演出する**：`src/main.ts`の`animateHits`は`hits`を「同じ攻撃者・同じhitIndex」でグループ化し、グループ単位でアニメーションを再生する（1体ずつ順番に殴るのではなく、1回の振りで全体に同時ヒットする見た目にする。2026-07-16修正）
