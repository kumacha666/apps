...
- **「巨大化」を廃止**、「特大化」はHP専用（`u.maxHp *= 3`、ATK項を削除）に戻す
- **「先鋭化」「硬質化」を新設**：単体選択、それぞれ`u.atk *= 3`／`u.def *= 3`のみ
- **`Unit`に`def`フィールドを新設**（初期値5、`units.ts`に`avgDef()`も追加）。被ダメ倍率は`dmgTakenMult = BASE_DEF / (BASE_DEF + def)`（`BASE_DEF`仮値40）で導出し、挑発からは完全に切り離す
- **「挑発」を全面再設計**：被ダメ軽減%は廃止し、「1ラウンドにつきtauntLevel回ぶんダメージを完全無効化（0ダメージ）、ラウンドごとにリフィル」という新メカニクスに変更。実装上の重要な注意点（掘り下げ済み、必ず`GAME_DESIGN.md`§7・§2.5を先に読むこと）：
  - ブロック予算は`enemyAttackTurn`内でラウンド単位のローカルMapとして管理し、複数の敵攻撃者にまたがって共有する
...
- **「全体攻撃化」の威力%上限を撤廃**：Lv6以降も対数減衰しながら伸び続ける（`combat.ts`の`aoeMultFor`を`GAME_DESIGN.md`§2.3の新式に置き換え）
