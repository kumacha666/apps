...
## カード全面再設計（2026-07-16実装。DEF・カード再編・挑発リワークは実装済み、全体攻撃化の上限撤廃と視覚システムは未実装）

ai-workspace側`GAME_DESIGN.md`（§2.3・§2.4・§2.5・§2.6・§2.8・§3・§3.1・§3.2・§7）で仕様が固まった大規模変更。DEF導入・特大化/先鋭化/硬質化・挑発の全面リワーク・反撃へのDEF適用・合体/分裂のDEF継承は実装済み（`types.ts`/`units.ts`/`combat.ts`/`battle.ts`/`data/cards.ts`/`main.ts`、テストは`units.test.ts`/`battle.test.ts`/`data/cards.test.ts`に追加）。**全体攻撃化の上限撤廃、および見た目（HP=サイズ/ATK=形/DEF=材質）とギャラリー機能は依然未着手**（下記参照）。
...
- **「全体攻撃化」の威力%上限撤廃は未実装（次の実装候補）**：以下の2箇所を**両方**新しい式に置き換える（片方だけ直すと「実ダメージは伸び続けるのにカード説明文だけ150%止まり」という表示不整合になる。2026-07-16、Codexレビュー指摘）：
...
- カード全面再設計（9→10種、DEF導入・挑発リワーク）は2026-07-16実装済み。**全体攻撃化の上限撤廃（`aoePercentForLevel`への置き換え）と、見た目（HP=サイズ/ATK=形/DEF=材質）・ギャラリー機能は未実装**（上記「カード全面再設計」セクション参照）。拡張候補（カウンタータンクのシナジー、回数制バリア持ちの敵）は`ai-workspace`側のGAME_DESIGN.md §11・§14を参照
