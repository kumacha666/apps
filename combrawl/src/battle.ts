...
/**
 * 「選んだユニットが、敵ターン終了後、味方が受けた攻撃の回数ぶんまとめて反撃する」
 * （カード説明文の通り）。反撃持ちユニット自身が狙われた時だけでなく、他の味方が被弾した
 * 時にも反撃が発動する。呼び出し元（`main.ts`）は`enemyAttackTurn()`の全ヒットのアニメーション
 * 再生が終わってからこの関数を呼ぶため、演出としては敵ターン終了後にまとめて反撃が飛ぶ
 * （被弾1回ごとに即座に割り込んで反撃するわけではない。2026-07-18、ユーザー指摘）。
 *
